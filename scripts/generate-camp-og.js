...
async function generateCampOgImage() {
  const rootDir = path.resolve(__dirname, "..");
  const campDir = path.join(rootDir, "camp");
  const sourceHtmlPath = path.join(campDir, "og-share.html");
  const outputImagePath = path.join(campDir, "images", "camp-og-share.jpg");
  const tempImagePath = path.join(campDir, "images", "camp-og-share-source.png");
  ...
  try {
    ...
    console.log(`Generated OG image: ${outputImagePath}`);
  } finally {
    await browser.close();
  }
}

generateCampOgImage().catch((error) => {
  console.error("Failed to generate camp OG image:", error.message);
  process.exitCode = 1;
});
